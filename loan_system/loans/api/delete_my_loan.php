...
/**
 * Delete My Loan API
 * Allows users to delete their own loan applications (only if pending)
 */
...
try {
    ...
    if ($loan['status'] !== 'pending') {
        http_response_code(400);
        echo json_encode(['success' => false, 'message' => 'Cannot delete loan application. Only pending applications can be deleted.']);
        exit;
    }
    
    // Get file paths before deletion (for cleanup if needed)
    ...
    $stmt = $pdo->prepare("
        ...
        VALUES (?, ?, ?, ?)
    ");
    $action = strtoupper($loanType) . '_DELETED';
    $description = "User deleted loan application #$loanId ($loanType)";
    ...
    http_response_code(500);
    echo json_encode(['success' => false, 'message' => 'Failed to delete loan application. Please try again.']);
}
